...
def start(input_image_folder, input_image_id, k):
    ...
    for filename in os.listdir(input_image_folder):
        ...
        hog_image_similarity = similarity_metrics.cosine(current_image_descriptor['hog']['hog_image'],
                                                         hog_data['hog_image'])
        image_scores = [cm_similarity, elbp_similarity, hog_histogram_similarity, hog_image_similarity]
        overall_score = similarity_metrics.l2_norm1([float(i) / sum(image_scores) for i in image_scores])
        heapq.heappush(h, (-overall_score, image))
        if len(h) > k:
            heapq.heappop(h)
